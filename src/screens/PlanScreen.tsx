        ...
        {WEEK_PLAN.map((dayPlan, dayIdx) => {
          ...
          return (
            <View key={dayIdx} style={[
              styles.dayCard,
              isToday && { borderColor: colors.critical + '60', backgroundColor: colors.bgElevated },
            ]}>
              <View style={styles.dayHeader}>
                <View style={styles.dayHeaderLeft}>
                  <View style={[
                    styles.dayBadge,
                    isToday && { backgroundColor: colors.critical, borderColor: colors.critical },
                  ]}>
                    <Text style={[styles.dayBadgeText, isToday && { color: colors.white }]}>
                  ...
                  {isToday && (
                    <View style={[styles.todayTag, {
                      ...
                      <Text style={[styles.todayTagText, { color: colors.critical }]}>DZIŚ</Text>
                    </View>
                  )}
                </View>
              ...
              {exercises.map(ex => (
                <View key={ex.id} style={styles.exerciseRow}>
                  <View style={[styles.dot, { backgroundColor: categoryColors[ex.category] }]} />
                  <Text style={styles.exerciseEmoji}>{ex.emoji}</Text>
                  <Text style={styles.exerciseName}>{ex.name}</Text>
                  <Text style={styles.exerciseTime}>
                    {ex.timeMin}{ex.timeMin !== ex.timeMax ? `–${ex.timeMax}` : ''} min
                  </Text>
                </View>
              ))}
              ...
              <View style={styles.catChips}>
                {exercises.map(ex => {
                  ...
                  return (
                    <View key={ex.id} style={[styles.catChip, {
...
function makeStyles(colors: ColorScheme) {
  return StyleSheet.create({
    ...
    exerciseRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 3 },
    ...
    catChipText: { fontSize: FontSize.xs, fontWeight: '600', letterSpacing: 0.5 },
  });
}
